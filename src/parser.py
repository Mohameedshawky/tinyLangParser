...
class Parser:
    ...
    def set_tokens_list_and_code_list(self, tokens_list, code_list):
        # Extract only the token values (first element of each tuple)
        self.tokens_list = [token[1] for token in tokens_list]
        self.tokens_list = [token.lower() for token in self.tokens_list]
    ...
    def match(self, x):
        # print(f"Matching: Expected '{x}', Found '{self.token.lower()}'")
        if self.token.lower() == x:
            self.next_token()
            return True
        else:
    ...
    def statement(self):
        if self.token.lower() == 'if':
            t = self.if_stmt()
            return t
        elif self.token.lower() == 'repeat':
            t = self.repeat_stmt()
            return t
        elif self.token.lower() == 'identifier':
            t = self.assign_stmt()
            return t
        elif self.token.lower() == 'read':
            t = self.read_stmt()
            return t
        elif self.token.lower() == 'write':
            t = self.write_stmt()
            return t
        else:
